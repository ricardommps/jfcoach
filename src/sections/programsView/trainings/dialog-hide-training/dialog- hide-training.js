...
import { StyledDialogActions } from '../dialog-publish-training/styles';

export function DialogHideTraining({ open, handleClose }) {
  return (
    <CustomizedDialogs onClose={handleClose} open={open}>
      <DialogTitle>
        <Typography sx={{ fontWeight: 800 }}>Ocultar Treino</Typography>
      </DialogTitle>
      <DialogContent>
        Os treinos selecionados serão ocultados para os alunos. Deseja prosseguir?
      </DialogContent>
      <StyledDialogActions>
        <LoadingButton
          color="info"
